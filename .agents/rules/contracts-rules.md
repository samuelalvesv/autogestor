...
- **gRPC Code-First & Protobuf**:
  - Toda a comunicação de dados entre clientes (Blazor WASM via gRPC-Web e futuros clientes nativos via HTTP/2) e o backend é realizada estritamente por gRPC Code-First via Protobuf, vedando a coexistência de controladores REST/JSON concorrentes ou adoção de serializadores binários divergentes.
