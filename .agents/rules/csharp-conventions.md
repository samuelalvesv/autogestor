...
- **Injeção de Dependências Mandatórias e Ausência de Opcionalidade**: Classes de serviço, persistência e infraestrutura gerenciadas pelo contêiner devem declarar suas dependências operacionais como estritamente obrigatórias no construtor primário. É proibida a utilização de tipos anuláveis ou valores padrão substitutivos para dependências essenciais, assegurando resolução integral pelo contêiner, validação estática em tempo de compilação e fail-fast imediato contra inicializações em estado inconsistente.
...
- **Caminho Único, Ausência de Fallbacks e Falha Rápida (Fail-Fast)**: É expressamente proibido fallbacks condicionais, valores padrão ou múltiplos caminhos concorrentes de resolução para configurações e contratos do sistema. O fluxo de execução e resolução de dependências deve ser estritamente único, canônico e mandatório, falhando imediatamente de forma explícita (fail-fast com exceção clara) caso o recurso ou parâmetro obrigatório não seja fornecido.
