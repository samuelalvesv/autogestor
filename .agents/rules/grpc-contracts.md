...
- As interfaces de contratos de serviço gRPC devem viver na camada `Autogestor.Contract` e ser decoradas com o atributo `[ServiceContract]`.
- Os DTOs de Request/Response gRPC devem ser declarados obrigatoriamente como `sealed record` decorados com `[DataContract]`, e cada propriedade exposta deve conter o atributo `[DataMember(Order = N)]` com numeração de ordem explícita.
