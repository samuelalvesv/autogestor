...
| --- | --- | --- | --- |
| **Contract** | `src/Autogestor.Contract` | Interfaces de serviços gRPC com atributos `[ServiceContract]`, DTOs de request e response decorados com `[DataContract]` e ordens explícitas. | Lógica de negócios, Handlers, dependências de banco de dados, pacotes pesados de infraestrutura. |
...
| **Application** | `src/Autogestor.Application` | Use Cases / Handlers (Commands, Queries), mapeamento de DTOs do `Contract` para o Domain, interfaces de serviços externos, validações de entrada. | Acesso a banco, serialização gRPC, Entity Framework. |
...
| **UI (RCL)** | `src/Autogestor.UI` | Componentes Razor compartilhados, páginas, layouts, CSS, consumo de serviços fortemente tipados do `Contract` injetados via DI. | Pacotes de hosting (WASM/MAUI), ServiceDefaults, configuração física de canais de comunicação. |
...
2. **Code-First:** Todos os contratos gRPC são definidos como interfaces C# clássicas e DTOs `record` dentro do projeto `Autogestor.Contract`. Isso elimina o uso de arquivos `.proto` e garante tipagem forte e segurança em tempo de compilação de ponta a ponta.
